...
export default function TrustStrip() {
  return (
    <section className="relative overflow-hidden bg-[#151512] px-5 py-20 md:px-8 md:py-36">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        >
          <div>
            ...
            <h2 className="max-w-4xl text-[clamp(2.6rem,12vw,7.5rem)] font-black leading-[0.96] tracking-tight text-white md:leading-[0.9]">
        ...
        <div className="mt-12 grid gap-10 md:mt-20 lg:grid-cols-[0.42fr_0.58fr] lg:items-start">
          <motion.div
            initial={{ opacity: 0, x: -28 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7 }}
            className="relative hidden min-h-[420px] overflow-hidden rounded-[8px] bg-[linear-gradient(145deg,rgba(255,255,255,0.08),rgba(255,255,255,0.015)),radial-gradient(circle_at_38%_18%,rgba(200,169,106,0.18),transparent_34%),radial-gradient(circle_at_70%_68%,rgba(138,164,173,0.16),transparent_36%)] md:block"
          >
          ...
          <div className="divide-y divide-white/8">
            {standards.map((standard, index) => (
              <motion.article
                ...
                className="grid gap-4 py-7 first:pt-0 last:pb-0 md:grid-cols-[96px_1fr] md:gap-6 md:py-9"
              >
                <p className="text-5xl font-black leading-none text-white/10">{String(index + 1).padStart(2, "0")}</p>
                <div>
                  <h3 className="text-2xl font-black tracking-tight text-white md:text-4xl">{standard.label}</h3>
                  <p className="mt-3 max-w-2xl text-base leading-relaxed text-white/56 md:mt-4 md:text-lg">{standard.copy}</p>
                </div>
              </motion.article>
            ))}
          </div>
