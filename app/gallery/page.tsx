            ...
            <article key={item.title} className={index === 0 ? "lg:col-span-2" : ""}>
              <div className={`relative overflow-hidden rounded-[8px] bg-black ${index === 0 ? "min-h-[620px]" : "min-h-[360px]"}`}>
                ...
                <div className="absolute bottom-7 left-7 right-7">
                  ...
                  <p className="mt-2 text-xs font-black uppercase tracking-[0.18em] text-neon-blue">{item.vehicle} · {item.category}</p>
                  <p className="mt-4 max-w-xl text-lg leading-relaxed text-white/64">{item.description}</p>
                </div>
              </div>
            </article>
          ))}
